                ...
                if M["m00"] != 0:
                    ...
                    if self.latest_depth_image is not None:
                        ...
                        if 0 <= center_y < depth_image.shape[0] and 0 <= center_x < depth_image.shape[1]:
                            ...
                            self.get_logger().info(f'Detected object at pixel: ({center_x}, {center_y}), Distance: {depth_value}')
                        else:
                            self.get_logger().warn('object centroid outside depth image bounds.')
                    else:
                        self.get_logger().warn('No depth image received yet.')
            else:
                # If no object is detected, you might want to publish a default value or nothing
                # point_msg = Point()
                # point_msg.x = -1.0
                # point_msg.y = -1.0
                # point_msg.z = -1.0
