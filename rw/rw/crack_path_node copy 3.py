...
class CrackPathNode(Node):
    def __init__(self):
        ...
        # Track how many raw clouds we've saved so far
    ...
    def pointcloud_callback(self, cloud_msg: PointCloud2):
        ...
        # --------------------------------------------------------
        # Save the first 3 raw point clouds to PCD
        # --------------------------------------------------------
        if self.frame_save_count < 3:
            try:
                # Convert raw numpy points to Open3D point cloud
                pcd_raw = o3d.geometry.PointCloud()
                pcd_raw.points = o3d.utility.Vector3dVector(points)

                # Construct the file path in your desired directory
                filename = os.path.join(self.save_directory, f"raw_cloud_{self.frame_save_count}.pcd")
                # Write to PCD file
                o3d.io.write_point_cloud(filename, pcd_raw)
                self.get_logger().info(f"Saved raw point cloud #{self.frame_save_count+1} to: {filename}")

                self.frame_save_count += 1
            except Exception as e:
                self.get_logger().error(f"Failed to save raw point cloud: {e}")

        # --------------------------------------------------------
        # Continue with the rest of your downsampling, segmentation, etc.
        # --------------------------------------------------------
        ...
        if inlier_points.shape[0] > 0:
            ...
            self.inlier_pub.publish(inlier_cloud_msg)  # Publish empty to clear
        ...
        if outlier_points.shape[0] > 0:
            ...
            self.outlier_pub.publish(outlier_cloud_msg)  # Publish empty to clear
