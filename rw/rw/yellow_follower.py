...
class YellowObjectFollower(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter("angular_chase_multiplier", 0.002)  # Reduced multiplier
        ...
        self.declare_parameter("filter_value", 0.7) # Slightly reduced filtering for responsiveness
        self.declare_parameter("image_width", 320)
        self.declare_parameter("max_offset_for_rotation", 50) # Threshold for rotation
        ...
        self.max_offset_for_rotation = self.get_parameter('max_offset_for_rotation').get_parameter_value().integer_value
    ...
    def control_loop(self):
        ...
        if time_diff < self.rcv_timeout_secs:
            ...
            # Angular control: Apply if distance is valid
            if self.target_distance != float('inf'):
                if abs(self.target_center_offset) > self.max_offset_for_rotation:
            ...
            # Linear control: Move forward if distance is valid and above threshold
            # Modified condition: Allow movement if a valid distance is received, even if initially high
            if self.target_distance != float('inf') and self.target_distance > self.min_distance_threshold:
                msg.linear.x = self.forward_chase_speed
            elif self.target_distance != float('inf') and self.target_distance <= self.min_distance_threshold:
                msg.linear.x = 0.0
